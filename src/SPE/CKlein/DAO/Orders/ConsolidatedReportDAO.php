...
class ConsolidatedReportDAO extends DwDbAbstract {
  ...
  public function getOrderSummary($orderIdsArray){
    ...
    $query = "select dw.*, sum(olsm.qty_shipped) as totalQtyShipped, group_concat(DISTINCT ol.state) as sOrderStatus, ws.customer_order_no as wmsOrderNumber, ws.shipped_date as wmsShippedDate, '' as wmsShippedQty, ws.tracking_number as wmsTrackingNumber, group_concat(DISTINCT po.po_number) as poNumbers from order_header oh, order_header_external_id_map oheim, order_line ol, order_line_shipment_map olsm, (select  dem.original_order_no as dwOriginalOrderNumber,  dem.order_date as dwOrderDate, dem.order_gross_price as dwOrderGrossPrice, sum(dpil.quantity) as dwTotalOrderedQty from dem_order_header dem, dem_product_item_line dpil where dem.order_header_id = dpil.order_header_id and dem.original_order_no in ".$orderIdsCsv. " group by dem.order_header_id) dw, ext_wms_shipment_report ws, purchase_order po where oh.order_header_id = oheim.order_header_id and ol.order_header_id = oh.order_header_id and olsm.order_line_id = ol.order_line_id and binary dw.dwOriginalOrderNumber = binary oheim.external_order_number and ws.customer_order_no = dw.dwOriginalOrderNumber and po.purchase_order_id = ol.purchase_order_id group by oh.order_header_id order by oh.order_header_id desc ";
